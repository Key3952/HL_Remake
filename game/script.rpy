...
# Переменные для подсчета доминирующих путей
default путь_ивлев = ""
default путь_агзамов = ""
default путь_рамзи = ""
...
label aperture_chaos:
    ...
    ai "Результаты неутешительны для статистики, но забавны для меня."

    # Дальше твой переход к следующей сцене
    jump aperture_memory
...
label pvz_defense:
    ...
    "Зомби наступают. Вы держите оборону."
...
label sonic_chase:
    ...
    menu:
        ...
        "Наперерез, короткий путь":
            $ эффективность += 1
            "Вы срезаете путь и почти догоняете Соника"
            
        "На максимальной скорости, напролом":
            $ скорость += 2
            $ хаос_фактор += 5
            gr "ДА! СКОРОСТЬ! СТРАСТЬ!"
            
        "Использовать гравипушку, чтобы поймать":
            $ хитрость += 1
            $ рецепты.append("гравитационный_захват")
            gr "ХИТРО! МНЕ НРАВИТСЯ!"
...
label final_boss:
    ...
    # Подсчёт очков для определения финала
    if точность > риск and эффективность > скорость:
        jump final_control_2
    elif риск > точность and атака > оборона:
        jump final_chaos_2
    elif основы > риск and основы > точность and хитрость > 0:
        jump final_peace_2
    elif хаос_фактор > порядок_фактор * 2:
        jump final_chaos_2
    elif порядок_фактор > хаос_фактор * 2:
        jump final_control_2
    else:
        jump final_balance
...
label final_diplomacy:
    ...
    menu:
        ...
        "Создать новый мир — идеальный, но живой":
            $ порядок_фактор += 10
            jump final_new_world
            
        "Разделить вселенные — твои и наши":
            $ союзники.append("габен_союзник")
            jump final_separation
            
        "Сделать Габена главным критиком":
            $ хаос_фактор += 5
            jump final_critic
...
label epilogue_2:
    ...
    if "идеальные_кубы" in рецепты:
        "— Идеальные кубы"
    if "хаотичные_формы" in рецепты:
        "— Хаотичные формы"
    if "понимание_техники" in рецепты:
        "— Понимание техники"
    if "импровизация" in рецепты:
        "— Импровизация"
    if "совместимость" in рецепты:
        "— Совместимость ингредиентов"
    if "организованный_хаос" in рецепты:
        "— Организованный хаос"
