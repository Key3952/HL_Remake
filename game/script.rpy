...
label pvz_defense_minigame:
    ...
    # Новая шкала оценки (максимум 15)
    if snake_score >= 14:
        r "Ого! Ты сожрал почти всех зомби! Это абсолютный рекорд!"
        $ оборона += 2
        $ отношение_агзамов += 15
        $ рецепты.append("змеиный_аппетит")
    elif snake_score >= 10:
        r "Неплохо! Зомби серьёзно поредели. Ты отлично справился!"
        $ оборона += 1
        $ отношение_агзамов += 10
        $ рецепты.append("хороший_аппетит")
    elif snake_score >= 5:
        r "Норм, зомби почти не осталось? Ну, по крайней мере, мы выжили."
        $ оборона += 0
        $ отношение_агзамов += 5
    else:
        r "Эх... зомби закусили тобой. Даже одного не смог сожрать!"
        menu:
            "Переиграть оборону":
                jump pvz_defense_minigame
...
label sonic_race_minigame:
    scene bg sonic_race with fade
    call minigame_sonic_runner
    $ sonic_score = _return   # количество собранных колец (0..10)

    # Оценка результата
    if sonic_score >= 10:
        # Отлично: собраны все кольца
        gr "ГОНЩИК! Ты собрал все кольца! Соник бы гордился!"
        $ скорость += 3
        $ отношение_рамзи += 15
        ...
        gr "Неплохо! Но Соник быстрее. Мог бы и больше колец собрать."
        $ скорость += 1
        $ отношение_рамзи += 5
    else:
        # Провал: мало колец
        gr "Эх... ты даже кольца собрать не смог! Соник разочарован."
        $ отношение_рамзи -= 10
        $ хаос_фактор += 3   # импровизация пошла не по плану
...
label final_boss:
    ...
    # Запуск финальной мини-игры
    call minigame_final_battle
    $ final_game_score = _return

    # Итоговый счёт и стиль определяют финал
    if final_game_score < 5:
        jump final_loser

    elif final_game_score >= 15:
        jump final_balance

    elif final_game_score >= 10:
        # высокий результат – финал по доминирующему стилю
        if точность >= риск and точность >= основы:
            jump final_control_2
        elif риск >= точность and риск >= основы:
            jump final_chaos_2
        else:
            jump final_peace_2

    else:  # final_game_score от 5 до 9
        # средний результат – дипломатия или предательство
        if хитрость >= 4 or "соник_друг" in союзники:
            jump final_diplomacy
        elif отношение_gman <= 30:
            jump final_betray
        else:
            # если нет явного выбора – идём к балансу с небольшим штрафом
            $ хаос_фактор += 1
            $ порядок_фактор -= 1
            jump final_balance
...
label final_loser:
    ...
    show gaben_glitch with dissolve
    gab "ВАШИ НАВЫКИ НИЧТОЖНЫ. ВЫ НЕ СПОСОБНЫ ДАЖЕ ПОНЯТЬ МОЙ АЛГОРИТМ."
    gab "ВЫ БЫЛИ ПЛОХИМ УЧЕНИКОМ И СТАНЕТЕ ПЛОХИМ ИНГРЕДИЕНТОМ."

    "Габен поглощает вас. Вы чувствуете, как ваша сущность растворяется в цифровом потоке."

    show gman_silhouette with dissolve
    g "Неожиданно... Я ожидал большего, доктор Фримен."
    g "Контракт нарушен. Вы потеряны."

    scene bg ending with fade
    "ФИНАЛ: АБСОЛЮТНАЯ НЕУДАЧА"
    "Вы не смогли объединить стили шеф-поваров. Габен переработал вас в безвкусный алгоритм."
    "Ивлев, Агзамов и Рамзи навсегда останутся запертыми в цифровой кухне."
    "G-Man исчезает, оставляя вас наедине с бесконечным циклом пересчёта калорий."

    $ рецепты.append("титул_неудачник")
    jump epilogue_2

# ЭПИЛОГ С ГРАФИЧЕСКИМ ИНТЕРФЕЙСОМ
label epilogue_2:
    scene black with fade
    play music "audio/epilogue.ogg" fadein 1.0

    # ---- Подсчёт максимального количества достижений ----
    $ max_skills = 9   # точность, риск, основы, оборона, атака, хитрость, скорость, эффективность, секретки
    $ skills_earned = (1 if точность > 0 else 0) + (1 if риск > 0 else 0) + (1 if основы > 0 else 0) + \
                      (1 if оборона > 0 else 0) + (1 if атака > 0 else 0) + (1 if хитрость > 0 else 0) + \
                      (1 if скорость > 0 else 0) + (1 if эффективность > 0 else 0) + (1 if секретки > 0 else 0)

    # Список всех возможных рецептов (для подсчёта максимума)
    $ all_recipes = ["гравипушка", "идеальная_нарезка", "термальное_мастерство", "текстурный_баланс", "дневник_ивлева",
                     "базовая_оборона", "огненная_атака", "хитрые_ловушки", "зомби-фьюжн", "безопасная_кухня",
                     "ледяной_горох", "взрывной_гранат", "золотой_подсолнух", "гравитационный_захват", "кольца_соника",
                     "укреплённая_оборона", "повелитель_хаоса", "синтез_совершенство", "синтез_средний", "синтез_провал",
                     "титул_архитектор", "титул_хаосит", "титул_наставник", "титул_синтезатор", "титул_предатель", "секретный_титул"]
    $ max_recipes = len(all_recipes)
    $ recipes_earned = sum(1 for r in all_recipes if r in рецепты)

    # Союзники (максимум 5)
    $ all_allies = ["защита", "фото_дневника", "кольца_соника", "соник_друг", "габен_союзник"]
    $ max_allies = len(all_allies)
    $ allies_earned = sum(1 for a in all_allies if a in союзники)

    # Общее количество достижений (сумма трёх категорий)
    $ total_earned = skills_earned + recipes_earned + allies_earned
    $ max_total = max_skills + max_recipes + max_allies

    # ---- ЭКРАН ЭПИЛОГА ----
    screen epilogue_screen():
        frame:
            ...
            vbox:
                spacing 15
                text "Итоги вашего путешествия" size 48 bold True color "#ffcc44" xalign 0.5
                hbox:
                    spacing 20
                    text "Общий прогресс:" size 24 color "#ffffff"
                    text "[total_earned] / [max_total]" size 24 color "#ffcc44" bold True
                bar value total_earned range max_total xsize 600 xalign 0.5
                ...
                viewport:
                    ...
                    vbox:
                        ...
                        frame:
                            ...
                            xfill True
                            vbox:
                                text "ПРОЙДЕННЫЕ ИСПЫТАНИЯ" size 28 bold True color "#88ff88"
                                hbox:
                                    text "Получено: [skills_earned] / [max_skills]" size 20 color "#cccccc"
                                null height 5
                                for skill_name, condition in [
                                    ("Мастер точности", точность > 0),
                                    ("Смелый экспериментатор", риск > 0),
                                    ("Хранитель традиций", основы > 0),
                                    ("Несокрушимый защитник", оборона > 0),
                                    ("Агрессивный новатор", атака > 0),
                                    ("Хитрый стратег", хитрость > 0),
                                    ("Скоростной шеф", скорость > 0),
                                    ("Эффективный менеджер", эффективность > 0),
                                    ("Искатель тайн", секретки > 0)
                                ]:
                                    hbox:
                                        spacing 10
                                        if condition:
                                            text "✓" color "#88ff88" size 24
                                            text skill_name color "#ffffff" size 20
                                        else:
                                            text "✗" color "#ff8888" size 24
                                            text skill_name color "#aaaaaa" size 20
                        ...
                        frame:
                            ...
                            xfill True
                            vbox:
                                text "СОБРАННЫЕ РЕЦЕПТЫ" size 28 bold True color "#88aaff"
                                hbox:
                                    text "Получено: [recipes_earned] / [max_recipes]" size 20 color "#cccccc"
                                null height 5
                                for recipe_name, recipe_id in [
                                    ("Гравипушка для взбивания", "гравипушка"),
                                    ("Идеальная нарезка", "идеальная_нарезка"),
                                    ("Термальное мастерство", "термальное_мастерство"),
                                    ("Текстурный баланс", "текстурный_баланс"),
                                    ("Дневник Ивлева", "дневник_ивлева"),
                                    ("Базовая оборона", "базовая_оборона"),
                                    ("Огненная атака", "огненная_атака"),
                                    ("Хитрые ловушки", "хитрые_ловушки"),
                                    ("Зомби-фьюжн", "зомби-фьюжн"),
                                    ("Безопасная кухня", "безопасная_кухня"),
                                    ("Ледяной горох", "ледяной_горох"),
                                    ("Взрывной гранат", "взрывной_гранат"),
                                    ("Золотой подсолнух", "золотой_подсолнух"),
                                    ("Гравитационный захват", "гравитационный_захват"),
                                    ("Кольца Соника", "кольца_соника"),
                                    ("Укреплённая оборона", "укреплённая_оборона"),
                                    ("Повелитель хаоса", "повелитель_хаоса"),
                                    ("Синтез совершенства", "синтез_совершенство"),
                                    ("Синтез средний", "синтез_средний"),
                                    ("Синтез провал", "синтез_провал"),
                                    ("Титул Архитектора", "титул_архитектор"),
                                    ("Титул Хаосита", "титул_хаосит"),
                                    ("Титул Наставника", "титул_наставник"),
                                    ("Титул Синтезатора", "титул_синтезатор"),
                                    ("Титул Предателя", "титул_предатель"),
                                    ("Секретный титул", "секретный_титул")
                                ]:
                                    hbox:
                                        spacing 10
                                        if recipe_id in рецепты:
                                            text "✓" color "#88ff88" size 24
                                            text recipe_name color "#ffffff" size 20
                                        else:
                                            text "✗" color "#ff8888" size 24
                                            text recipe_name color "#aaaaaa" size 20

                        # Союзники
                        frame:
                            background Solid("#3a2a2a")
                            xfill True
                            vbox:
                                text "СОЮЗНИКИ" size 28 bold True color "#ffaa88"
                                hbox:
                                    text "Получено: [allies_earned] / [max_allies]" size 20 color "#cccccc"
                                null height 5
                                for ally_name, ally_id in [
                                    ("Защитный костюм", "защита"),
                                    ("Фото дневника Ивлева", "фото_дневника"),
                                    ("Кольца Соника", "кольца_соника"),
                                    ("Соник (друг)", "соник_друг"),
                                    ("Габен (союзник)", "габен_союзник")
                                ]:
                                    hbox:
                                        spacing 10
                                        if ally_id in союзники:
                                            text "✓" color "#88ff88" size 24
                                            text ally_name color "#ffffff" size 20
                                        else:
                                            text "✗" color "#ff8888" size 24
                                            text ally_name color "#aaaaaa" size 20

                null height 20
                textbutton "Продолжить" action Return() xalign 0.5 xsize 200

    call screen epilogue_screen

    # ---- Дальнейшие диалоги G-Man ----
    show gman_normal with dissolve
